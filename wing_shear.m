...
section1    = struct('num_str', 0,'start_pos', spar1.position(1), 'end_pos', spar2.position(1), 'x_length', 0, 'web_thick', 0, 'stringers', zeros(num_stringers(1),3), 'webs', [], 'areas', zeros(num_stringers(1)+1,1));
section2    = struct('num_str', 0,'start_pos', spar1.position(1), 'end_pos', spar2.position(1), 'x_length', 0, 'web_thick', 0, 'stringers', zeros(num_stringers(2),3), 'webs', [], 'areas', zeros(num_stringers(2)+1,1)); 
section3    = struct('num_str', 0,'start_pos', 0                , 'end_pos', spar1.position(1), 'x_length', 0, 'web_thick', 0, 'stringers', zeros(num_stringers(3),3), 'webs', [], 'areas', zeros(num_stringers(3)+1,1)); 
section4    = struct('num_str', 0,'start_pos', 0                , 'end_pos', spar1.position(1), 'x_length', 0, 'web_thick', 0, 'stringers', zeros(num_stringers(4),3), 'webs', [], 'areas', zeros(num_stringers(4)+1,1)); 
...
for index1 = 1:num_spars
    I_x = I_x + wing.spars(index1).area*(wing.spars(index1).position(z_pos)^2); 
end 
...
for index1 = 1:num_spars
    I_z = I_z + wing.spars(index1).area*(wing.spars(index1).position(x_pos)^2); 
end 
...
for index1 = 1:num_sections
    for index2 = 1:(num_stringers(index1)+1)
        if index2 == 1
            ...
            wing.sections(index1).areas(index2) = get_z(x_i,mod(index1,2))*(x_i-x_minus)/2 - (get_z(x_plus,mod(index1,2))*(x_plus-x_minus))/2 + integral(end);
